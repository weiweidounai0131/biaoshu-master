...
class DeliveryServerTest(unittest.TestCase):
    ...
    def test_reader_returns_registered_source_in_pages(self) -> None:
        ...
        status, rechecking = self.request("/api/overview")
        self.assertEqual(status, 200)
        self.assertEqual(rechecking["overview"]["batches"][0]["status"], "ai_rechecking")
        self.assertFalse(rechecking["overview"]["batches"][0]["readable"])
        self.assertEqual(rechecking["overview"]["batches"][0]["ai_recheck"]["status"], "running")
        status, report = self.request("/api/batches/word-batch-1/ai-recheck")
        self.assertEqual(status, 200)
        self.assertEqual(report["ai_recheck"]["report"]["status"], "running")
        self.fixture._pass_ai_recheck("word-batch-1")
        status, overview = self.request("/api/overview")
        self.assertEqual(status, 200)
    ...
    def test_web_review_actions_persist_without_calling_ai(self) -> None:
        ...
        self.assertEqual(edited["export"]["manifest"]["status"], "ai_rechecking")
        self.fixture._pass_ai_recheck("word-batch-1")
    ...
    def test_final_delivery_requires_wps_page_check(self) -> None:
        ...
        self.fixture._pass_ai_recheck("word-batch-2")
    ...
    def test_http_batch_and_final_confirmation_flow(self) -> None:
        ...
        for batch_id, pages in (("word-batch-1", 9), ("word-batch-2", 10)):
            ...
            self.fixture._pass_ai_recheck(batch_id)
