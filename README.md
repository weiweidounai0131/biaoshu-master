...
| 分批与并行 Word 生产 | 长文一次输出难以检查，串行等待时间较长 | 1 至 5 个按授权生成的 Word 批次；宿主支持时可并行生产 |
...
调用后，AI 会先打开本地确认入口，区分背景资料和参考资料，再按四个阶段推进。每个大阶段结束都会停在确认闸门；只有用户明确确认或继续，才进入下一个阶段。Stage4 选择多个 Word 批次时，会先检查当前宿主是否支持安全的子代理并发；支持时可并行生产，之后仍须完成全部批次的 AI 复校和人工审阅，不支持时自动回到串行流程。用户可以在阶段页面直接修改字段，提交后的前端回执优先于 Skill 默认值。
...
- [Stage4 并行批次生产协议](references/parallel-batch-production.md)：子代理能力检查、隔离生产、统一复校和降级处理；
